...
export class NoteCreationUseCase {
	...
	async createProjectFolder(
		...
		logger.debug(
			`[UseCase:start] NoteCreationUseCase.createProjectFolder parent=${request.parentPath}`,
		);
		...
		logger.debug(
			`[UseCase:end] NoteCreationUseCase.createProjectFolder path=${folder.path}`,
		);
	...
	async createProjectNote(
		...
		logger.debug(
			`[UseCase:start] NoteCreationUseCase.createProjectNote parent=${request.parentPath}`,
		);
		...
		const note = new NoteSummary({
			notePath: joinPath(
				request.parentPath,
				`${prefix}_${request.title.trim()}.md`,
			),
	...
	private buildTodayDailyNoteLink(): string {
		...
		const notePath = this.runtime
			.resolveNoteUri(today)
			.replace(/\.md$/i, "");
